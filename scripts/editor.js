...
function setDrawingMode(mode) {
  ...
  document.querySelectorAll('.tools button').forEach((btn) => {
    if (btn.textContent.includes(labels[mode])) btn.classList.add('mode-active');
  });
...
function clearAll() {
  ...
  [...entranceMarkers, ...exitMarkers, ...dockMarkers].forEach((m) => {
    m.map = null;
  });
  entranceMarkers = [];
  exitMarkers = [];
...
function getMarkerLabel(marker, fallback) {
  return marker.__label || fallback;
}
...
function restoreMapFromData(data) {
  ...
  data.truckExits?.forEach((pt) => {
    exitMarkers.push(
      createEditorMarker({ lat: pt.lat, lng: pt.lng }, pt.label || 'EXIT', 'exit')
    );
  });
...
function buildLocationData() {
  ...
  const detailData = {
    ...
    truckEntrances: entranceMarkers.map((m) => {
      ...
      return { lat: p.lat, lng: p.lng, label: getMarkerLabel(m, 'ENTRANCE') };
    }),
    truckExits: exitMarkers.map((m) => {
      const p = getMarkerPosition(m);
      return { lat: p.lat, lng: p.lng, label: getMarkerLabel(m, 'EXIT') };
    }),
    docks: dockMarkers.map((m, i) => {
      ...
      return {
        lat: p.lat,
        lng: p.lng,
        name: getMarkerLabel(m, `Dock ${i + 1}`),
        notes: m.__notes || ''
      };
    }),
  ...
  const folderName = category === 'Store' ? 'Stores' : category === 'Terminal' ? 'Terminals' : 'Locations';
  ...
  return { detailData, masterEntry, fileName };
}

function formatJsonOutput(detailData, masterEntry, fileName) {
  const detailJson = JSON.stringify(detailData, null, 2);
  const masterJson = JSON.stringify(masterEntry, null, 2);
  return `--- Detail file: Locations/.../${fileName} ---\n${detailJson}\n\n--- Add to masterlist.json "locations" array ---\n${masterJson}`;
}

function updateJsonPreview() {
  const { detailData, masterEntry, fileName } = buildLocationData();
  document.getElementById('jsonOutput').textContent = formatJsonOutput(detailData, masterEntry, fileName);
  return { detailData, masterEntry, fileName };
}

function generateJSON() {
  const { detailData, fileName } = updateJsonPreview();
  downloadJson(JSON.stringify(detailData, null, 2), fileName);
}
...
function copyJSON() {
  ...
  if (!output || output === 'Generated JSON will appear here.') {
    updateJsonPreview();
    return copyJSON();
  }
